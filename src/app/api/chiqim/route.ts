...
export const GET = route(async (req: NextRequest) => {
  ...
  authorize(ctx, PERMISSIONS.CHIQIM_LIST);
  ...
  const where = {
    ...
    ...(q.search ? {
      OR: [
        { productName: { contains: q.search, mode: 'insensitive' as const } },
        { recipient: { contains: q.search, mode: 'insensitive' as const } },
        { responsible: { contains: q.search, mode: 'insensitive' as const } },
      ],
    } : {}),
    ...(q.dateFrom || q.dateTo ? {
      chiqimDate: {
  ...
  const [items, total] = await Promise.all([
    prisma.chiqim.findMany({ where, include, orderBy: { chiqimDate: 'desc' }, skip: (q.page - 1) * q.pageSize, take: q.pageSize }),
...
export const POST = route(async (req: NextRequest) => {
  const ctx = await authenticate(req);
  authorize(ctx, PERMISSIONS.CHIQIM_CREATE);

  const body = createSchema.parse(await req.json());
  const branchId = isGlobalRole(ctx.roleType) ? (body.branchId ?? ctx.branchId!) : ctx.branchId!;

  const item = await prisma.chiqim.create({
    data: {
      productName: body.productName ?? null,
      recipient: body.recipient,
      responsible: body.responsible,
      chiqimDate: body.chiqimDate ? new Date(body.chiqimDate) : new Date(),
